...
function buscarDispositivo(){
  loading.value=true
  setTimeout(()=>{loading.value=false,paired.value=true},3000)
}
function setIcon(){
      ...
      <v-stepper-vertical v-model="currentStep" theme="light">
        <v-stepper-vertical-item title="Paso 1" icon="mdi-numeric-1" :complete="(currentStep>0)">
          <v-card title="Selecciona el tipo de dispositivo" flat>
            <v-select label="Dispositivo" v-model="deviceTypeSelect" :items=deviceTypes item-title="name" item-value="id"
        ...
        <v-stepper-vertical-item title="Paso 2" icon="mdi-numeric-2" :complete="(currentStep>1)">
          <v-card title="Vincular dispositvo" flat>
            <v-card>
              <v-card-text>
                <v-btn :loading="loading" :color="loading? 'primary':'white'" @click="buscarDispositivo">Buscar</v-btn>
                <v-card v-if="paired" append-icon="mdi-check" flat color="green" :title="getNameDevicePaired()" ></v-card>
              </v-card-text>
            </v-card>
          </v-card>
          <template v-slot:next>
        ...
        <v-stepper-vertical-item title="Paso 3" icon="mdi-numeric-3" :complete="(currentStep>2)">
          <v-card title="Introduce un nombre para tu dispositivo" flat>
            <v-card-text>
              <v-text-field v-model="deviceName" counter="20" :rules="deviceNameRules" clearable label="Nombre"
                            placeholder="LAMPARA LIVING" hint="Entre 3-60 caracteres" />
            </v-card-text>
          </v-card>
          <template v-slot:next>
