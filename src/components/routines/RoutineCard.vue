...
const handleSecurity = () => {
   ...
   */
  if(pinStore.pin){
    protectedDevices.value = routine.actions.filter((a) => a.device.meta.protected).map((a) => a.device.name)
...
const handleExecute = async () => {
  ...
  try {
    ...
    routine.actions.forEach((act, index) => {
      theResults.value.push({deviceName : act.device.name, actionName : actionsReadableNames[act.actionName as keyof typeof actionsReadableNames ], done : results.result[index]})
    })
...
<template>
  <div class="container">
  <v-card class="card" min-height="10vh" :color="isSelected ? 'focus' : ''" @click="$emit('click', routine.id)">
    <v-card-item >
        <div class="flex">
            {{props.routine.name}}
        </div>
    ...
    <div class="executeBtn">
      <v-btn prepend-icon="mdi-play" variant="tonal"
    ...
    <v-card>
      ...
      <div class="dialogActions">
        <v-card-actions class="dialogActions">
          ...
          <v-btn color="primary" class="buttons" @click="()=> { confirmDialog = false; handleSecurity() }">Run Routine</v-btn>
        </v-card-actions>
      </div>
    </v-card>
  </v-dialog>
  ...
  >
    <v-card>
      ...
      <div class="dialog">
        <p>The "{{routine.name}}" routine is PIN protected. To continue, please insert the PIN.</p>
      </div>
      <v-otp-input
        :length="4"
        :error="!valid_pin"
  ...
  >
    <v-card>
        ...
        <div class="divRoutineResults">
          <ul class="routineResults">
            <li v-for=" (r, idx) in theResults" :key="idx + Math.floor(Math.random() *1000)">
              ...
              }} <v-icon :icon="r.done ?  'mdi-check' : (r.done === false ? 'mdi-close' : 'mdi-equal')"
      ...
      <div class="ml-4">
        <div class="referencesIconTitle" @click="seeReferences =!seeReferences">
        ...
        <div v-if="seeReferences">
          <ul class="routineReference" >
            ...
            <li><v-icon icon="mdi-close" color="error"/>Device current state was found incompatible with the request action</li>
          </ul>
        </div>
      </div>
      <div class="dialogActions">
